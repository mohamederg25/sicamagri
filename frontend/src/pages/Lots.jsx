...
const Lots = () => {
  ...
  const filteredLots = lots.filter(lot => {
    ...
    const typeMatch = typeFilter === 'all' || lot.type === typeFilter;
    const statusMatch = statusFilter === 'all' || lot.statut === statusFilter;

    if (!pepMatch || !varMatch || !typeMatch || !statusMatch) return false;
  ...
  const lotsProduction = filteredLots;
  ...
  const calculateRate = (lot) => {
    // Germination rate is snapped on the lot from the linked stock
    if (lot.tauxGermination != null) {
      return lot.tauxGermination;
    }
    return null;
  };

  const { sortedData: sortedLotsProd, handleSort: handleSortProd, SortIcon: SortIconProd } = useSort(lotsProduction, { defaultField: 'code' });
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      <header>
        <h1 style={{ fontSize: '36px', fontWeight: 800, color: '#111111', margin: 0 }}>Lots</h1>
        <p style={{ fontSize: '18px', color: '#222222', margin: 0 }}>Suivi des lots de semence et de production</p>
      </header>
...
<ExportButton
            user={user}
            filename="lots"
            columns={[{ accessor: 'code', header: 'Code' }, { accessor: 'type', header: 'Type' }, { accessor: 'semis.variete.nom', header: 'Variété' }, { accessor: 'semis.pepiniere.nom', header: 'Pépinière' }, { accessor: 'quantite', header: 'Quantité' }, { accessor: 'dateEntree', header: 'Date' }]}
            data={filteredLots}
            mapRow={(lot) => [lot.code || '-', 'Production', lot.semis?.variete?.nom || '-', lot.semis?.pepiniere?.nom || '-', lot.quantite?.toString() || '-', lot.dateEntree ? new Date(lot.dateEntree).toLocaleDateString('fr-FR') : '-']}
          />
        ...
        {lotsProduction.length > 0 && <>
          <div style={{ padding: '16px 24px', backgroundColor: '#f0fdf4', borderBottom: '1px solid #C8E6C9', borderTop: '2px solid #C8E6C9' }}>
            <h2 id="prod-lots-heading" style={{ fontSize: '20px', fontWeight: 700, color: '#006625', margin: 0 }}>
              Lots de production
              <span style={{ marginLeft: '10px', fontSize: '13px', fontWeight: 500, color: '#111111' }}>({lotsProduction.length})</span>
            </h2>
          </div>
          <div className="table-scroll" style={{ overflowX: 'auto' }}>
          <table aria-labelledby="prod-lots-heading" style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ backgroundColor: '#f9fafb' }}>
                <th scope="col" onClick={() => handleSortProd('code')} style={{ cursor: 'pointer', textAlign: 'center', padding: '20px 24px', fontSize: '14px', fontWeight: 700, color: '#111111', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid #C8E6C9', userSelect: 'none' }}>Code<SortIconProd field="code" /></th>
                <th scope="col" onClick={() => handleSortProd('lotSemenceParent.code')} style={{ cursor: 'pointer', textAlign: 'center', padding: '20px 24px', fontSize: '14px', fontWeight: 700, color: '#111111', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid #C8E6C9', userSelect: 'none' }}>Lot semence<SortIconProd field="lotSemenceParent.code" /></th>
                <th scope="col" onClick={() => handleSortProd('semis.variete.nom')} style={{ cursor: 'pointer', textAlign: 'center', padding: '20px 24px', fontSize: '14px', fontWeight: 700, color: '#111111', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid #C8E6C9', userSelect: 'none' }}>Variété<SortIconProd field="semis.variete.nom" /></th>
                <th scope="col" onClick={() => handleSortProd('semis.pepiniere.nom')} style={{ cursor: 'pointer', textAlign: 'center', padding: '20px 24px', fontSize: '14px', fontWeight: 700, color: '#111111', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid #C8E6C9', userSelect: 'none' }}>Pépinière<SortIconProd field="semis.pepiniere.nom" /></th>
                <th scope="col" onClick={() => handleSortProd('statut')} style={{ cursor: 'pointer', textAlign: 'center', padding: '20px 24px', fontSize: '14px', fontWeight: 700, color: '#111111', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid #C8E6C9', userSelect: 'none' }}>Statut<SortIconProd field="statut" /></th>
                <th scope="col" onClick={() => handleSortProd('quantite')} style={{ cursor: 'pointer', textAlign: 'center', padding: '20px 24px', fontSize: '14px', fontWeight: 700, color: '#111111', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid #C8E6C9', userSelect: 'none' }}>A planter<SortIconProd field="quantite" /></th>
                <th scope="col" onClick={() => handleSortProd('dateEntree')} style={{ cursor: 'pointer', textAlign: 'center', padding: '20px 24px', fontSize: '14px', fontWeight: 700, color: '#111111', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid #C8E6C9', userSelect: 'none' }}>Date<SortIconProd field="dateEntree" /></th>
        ...
        {/* Empty state when all filters active but nothing found */}
        {lotsProduction.length === 0 && (
          <div style={{ padding: '60px 40px', textAlign: 'center', color: '#222222' }}>
            <p style={{ fontSize: '18px', fontWeight: 600, margin: 0 }}>
              {searchTerm || pepFilter !== 'all' || varFilter !== 'all' || typeFilter !== 'all' || statusFilter !== 'all'
